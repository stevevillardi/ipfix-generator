import sys
import argparse
import random
import socket
import struct
import time
import ipaddress
import csv
import os
import signal
import warnings
import logging
from io import BytesIO
import base64

# Suppress Scapy runtime warnings
logging.getLogger("scapy.runtime").setLevel(logging.ERROR)
warnings.filterwarnings("ignore", category=DeprecationWarning)

from scapy.all import IP, UDP, Raw, send, conf

# Configure Scapy to suppress verbose output
conf.verb = 0

from PyQt6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, 
                             QPushButton, QTextEdit, QSpinBox, QDoubleSpinBox, QTableWidget, 
                             QTableWidgetItem, QFileDialog, QSplitter, QStatusBar, QProgressBar,
                             QGroupBox, QGridLayout, QHeaderView)
from PyQt6.QtCore import (QThread, pyqtSignal, Qt, QTimer)
from PyQt6.QtGui import QPixmap, QImage, QColor, QBrush

LOGO_BASE64 = """

"""

def parse_ip_ranges(ip_ranges):
    """Parse comma-separated IP addresses and CIDR ranges.
    
    Args:
        ip_ranges: String containing comma-separated IPs and CIDR ranges
        
    Returns:
        List of IP addresses as strings
        
    Raises:
        ValueError: If any IP address or range is invalid
    """
    ip_list = []
    if not ip_ranges or not ip_ranges.strip():
        return ip_list
        
    for ip_range in ip_ranges.split(','):
        ip_range = ip_range.strip()
        if not ip_range:
            continue
            
        try:
            if '/' in ip_range:
                # Parse CIDR range
                network = ipaddress.IPv4Network(ip_range, strict=False)
                ip_list.extend([str(ip) for ip in network])
            else:
                # Validate single IP address
                ipaddress.IPv4Address(ip_range)
                ip_list.append(ip_range)
        except ValueError as e:
            raise ValueError(f"Invalid IP address or range '{ip_range}': {str(e)}")
            
    if not ip_list:
        raise ValueError("No valid IP addresses found in input")
        
    return ip_list

def generate_flows(flow_src_ips, flow_dst_ips, num_flows=1000, show_progress=False):
    """
    Generate realistic network flows with traffic patterns and sessions.
    
    Creates persistent sessions, realistic traffic profiles, and varied patterns
    to simulate authentic network behavior for NetFlow monitoring and visualization.
    """
    flows = []
    progress_interval = max(1, num_flows // 10)  # Show progress every 10%
    
    # Define realistic traffic profiles with weighted probabilities
    # Format: (port, protocol_name, protocol_number, weight)
    # Higher weight = more common in real networks
    traffic_profiles = {
        'web': [
            (80, "HTTP", 6, 35),      # Very common - web browsing
            (443, "HTTPS", 6, 50),    # Most common - encrypted web
            (8080, "HTTP Alt", 6, 5), # Alternative HTTP
            (8443, "HTTPS Alt", 6, 3) # Alternative HTTPS
        ],
        'database': [
            (3306, "MySQL", 6, 40),       # Popular database
            (5432, "PostgreSQL", 6, 30),  # Common in enterprise
            (1433, "MS SQL", 6, 20),      # Microsoft environments
            (1521, "Oracle", 6, 10)       # Enterprise database
        ],
        'email': [
            (25, "SMTP", 6, 30),      # Sending email
            (587, "SMTP", 6, 20),     # SMTP submission
            (143, "IMAP", 6, 25),     # Reading email
            (993, "IMAPS", 6, 15),    # Secure IMAP
            (110, "POP3", 6, 7),      # Legacy email
            (995, "POP3S", 6, 3)      # Secure POP3
        ],
        'infrastructure': [
            (53, "DNS", 17, 60),      # Very frequent - UDP
            (53, "DNS", 6, 10),       # Occasional - TCP
            (123, "NTP", 17, 20),     # Time sync
            (514, "Syslog", 17, 10)   # Logging
        ],
        'remote_access': [
            (22, "SSH", 6, 60),       # Linux/Unix access
            (3389, "RDP", 6, 40)      # Windows access
        ],
        'other': [
            (389, "LDAP", 6, 20),     # Directory services
            (636, "LDAPS", 6, 15),    # Secure LDAP
            (5060, "SIP", 17, 10),    # VoIP
            (161, "SNMP", 17, 5)      # Network monitoring
        ]
    }
    
    # Create weighted profile distribution (web traffic dominates)
    profile_weights = {
        'web': 50,            # 50% web traffic
        'infrastructure': 20, # 20% DNS/NTP
        'database': 10,       # 10% database
        'email': 8,           # 8% email
        'remote_access': 7,   # 7% SSH/RDP
        'other': 5            # 5% other services
    }
    
    # Create persistent sessions (10-20% of flows are from recurring sessions)
    num_sessions = max(10, num_flows // 20)  # ~5% of flows as unique sessions
    sessions = []
    
    # Pre-generate some persistent sessions
    for _ in range(num_sessions):
        profile_name = random.choices(
            list(profile_weights.keys()),
            weights=list(profile_weights.values())
        )[0]
        
        profile = traffic_profiles[profile_name]
        service = random.choices(
            profile,
            weights=[s[3] for s in profile]
        )[0]
        
        src_ip = random.choice(flow_src_ips) if flow_src_ips else \
            f"{random.randint(10, 192)}.{random.randint(0, 255)}.{random.randint(0, 255)}.{random.randint(1, 254)}"
        dst_ip = random.choice(flow_dst_ips) if flow_dst_ips else \
            f"{random.randint(10, 192)}.{random.randint(0, 255)}.{random.randint(0, 255)}.{random.randint(1, 254)}"
        
        sessions.append({
            'src_ip': src_ip,
            'dst_ip': dst_ip,
            'dst_port': service[0],
            'protocol': service[2],
            'profile': profile_name,
            'repeat_count': random.randint(3, 15)  # How many times this session appears
        })
    
    # Generate flows with realistic patterns
    session_index = 0
    for i in range(num_flows):
        if show_progress and i > 0 and i % progress_interval == 0:
            percent = (i / num_flows) * 100
            print(f"  Progress: {percent:.0f}% ({i}/{num_flows} flows)", end='\r', flush=True)
        
        # 20% chance to use a persistent session (makes traffic look realistic)
        use_session = random.random() < 0.20 and sessions
        
        if use_session:
            # Use an existing session
            session = sessions[session_index % len(sessions)]
            session_index += 1
            
            flow = {
                'src_ip': session['src_ip'],
                'dst_ip': session['dst_ip'],
                'dst_port': session['dst_port'],
                'protocol': session['protocol'],
                'ingress_interface': random.randint(1, 10),  # Fewer interfaces = more realistic
                'egress_interface': random.randint(1, 10)
            }
        else:
            # Generate a new flow
            profile_name = random.choices(
                list(profile_weights.keys()),
                weights=list(profile_weights.values())
            )[0]
            
            profile = traffic_profiles[profile_name]
            service = random.choices(
                profile,
                weights=[s[3] for s in profile]
            )[0]
            
            # Generate IPs
            if flow_src_ips:
                src_ip = random.choice(flow_src_ips)
            else:
                # Use private IP ranges for more realistic src IPs
                src_ip = f"192.168.{random.randint(0, 255)}.{random.randint(1, 254)}"
            
            if flow_dst_ips:
                dst_ip = random.choice(flow_dst_ips)
            else:
                # Use mix of private and public-looking IPs for destinations
                if random.random() < 0.3:  # 30% internal
                    dst_ip = f"10.{random.randint(0, 255)}.{random.randint(0, 255)}.{random.randint(1, 254)}"
                else:  # 70% external
                    dst_ip = f"{random.randint(1, 223)}.{random.randint(0, 255)}.{random.randint(0, 255)}.{random.randint(1, 254)}"
            
            flow = {
                'src_ip': src_ip,
                'dst_ip': dst_ip,
                'dst_port': service[0],
                'protocol': service[2],
                'ingress_interface': random.randint(1, 10),  # Concentrate on fewer interfaces
                'egress_interface': random.randint(1, 10)
            }
        
        flows.append(flow)
    
    if show_progress:
        print()  # Clear the progress line
    
    return flows

def get_realistic_traffic_volume(dst_port, protocol):
    """
    Generate realistic packet and byte counts based on service type.
    Returns (packets, avg_packet_size) tuple.
    """
    # Define realistic traffic patterns for different services
    # Format: (min_packets, max_packets, min_bytes_per_packet, max_bytes_per_packet)
    traffic_patterns = {
        # DNS - small, quick queries
        53: (1, 5, 50, 512),
        
        # HTTP - varied, can be small or large
        80: (5, 200, 200, 1500),
        8080: (5, 200, 200, 1500),
        
        # HTTPS - encrypted, slightly more overhead
        443: (10, 500, 200, 1500),
        8443: (10, 500, 200, 1500),
        
        # Email - varies by operation
        25: (5, 50, 100, 1000),    # SMTP sending
        587: (5, 50, 100, 1000),   # SMTP submission
        110: (3, 30, 100, 800),    # POP3 download
        995: (3, 30, 100, 800),    # POP3S download
        143: (5, 100, 100, 1200),  # IMAP operations
        993: (5, 100, 100, 1200),  # IMAPS operations
        
        # Database - medium queries
        3306: (5, 100, 100, 1400),   # MySQL
        5432: (5, 100, 100, 1400),   # PostgreSQL
        1433: (5, 100, 100, 1400),   # MS SQL
        1521: (5, 100, 100, 1400),   # Oracle
        
        # Remote access - interactive, steady
        22: (20, 500, 64, 800),      # SSH interactive
        3389: (50, 1000, 100, 1400), # RDP - more overhead
        
        # File transfers - large
        20: (100, 5000, 500, 1500),  # FTP data
        21: (5, 50, 64, 500),        # FTP control
        
        # Infrastructure services
        123: (1, 3, 48, 90),         # NTP - tiny
        161: (1, 10, 64, 484),       # SNMP
        514: (1, 20, 100, 1024),     # Syslog
        
        # Directory services
        389: (3, 50, 100, 1000),     # LDAP
        636: (3, 50, 100, 1000),     # LDAPS
        
        # VoIP
        5060: (100, 2000, 100, 300), # SIP - many small packets
        5061: (100, 2000, 100, 300), # SIP TLS
    }
    
    # Get pattern for this port, or use default
    if dst_port in traffic_patterns:
        min_pkt, max_pkt, min_size, max_size = traffic_patterns[dst_port]
    else:
        # Default pattern for unknown services
        min_pkt, max_pkt, min_size, max_size = (5, 100, 100, 1200)
    
    # Generate packet count with some randomness
    packets = random.randint(min_pkt, max_pkt)
    
    # Generate average packet size
    avg_packet_size = random.randint(min_size, max_size)
    
    # Add some variation for realism (not all packets same size)
    # Calculate total bytes with ±20% variation per packet
    total_bytes = sum(
        random.randint(
            int(avg_packet_size * 0.8),
            int(avg_packet_size * 1.2)
        ) for _ in range(packets)
    )
    
    return packets, total_bytes

class IPFIXGeneratorThread(QThread):
    update_signal = pyqtSignal(str)
    flow_signal = pyqtSignal(dict)
    stats_signal = pyqtSignal(int)  # Total flows sent
    profile_signal = pyqtSignal(dict)  # Traffic profile statistics
    rate_signal = pyqtSignal(float, float)  # flows/sec, packets/sec

    def __init__(self, source_ips, destination_ip, destination_port, num_flows, interval, packets_per_interval, flows_per_packet, flow_src_ips, flow_dst_ips, flows):
        QThread.__init__(self)
        self.source_ips = source_ips
        self.destination_ip = destination_ip
        self.destination_port = destination_port
        self.num_flows = num_flows
        self.interval = interval
        self.flows = flows
        self.packets_per_interval = packets_per_interval
        self.flows_per_packet = flows_per_packet
        self.flow_src_ips = flow_src_ips
        self.flow_dst_ips = flow_dst_ips
        self.is_running = True
        self.template_refresh_interval = 60  # Send template every 60 seconds
        self.sequence_number = 0
        self.last_template_time = 0
        self.observation_domain_id = random.randint(0, 2**32 - 1)
        self.total_flows_sent = 0  # Total flows sent counter
        self.current_flow_index = 0
        self.start_time = time.time()
        self.last_rate_time = time.time()
        self.flows_since_last_rate = 0
        self.packets_sent = 0
        # Traffic profile counters
        self.profile_counts = {
            'web': 0, 'infrastructure': 0, 'database': 0,
            'email': 0, 'remote_access': 0, 'other': 0
        }

    def categorize_flow_by_port(self, port):
        """Categorize flow into traffic profile based on destination port"""
        web_ports = [80, 443, 8080, 8443]
        infra_ports = [53, 123, 514]
        db_ports = [3306, 5432, 1433, 1521]
        email_ports = [25, 587, 143, 993, 110, 995]
        remote_ports = [22, 3389]
        
        if port in web_ports:
            return 'web'
        elif port in infra_ports:
            return 'infrastructure'
        elif port in db_ports:
            return 'database'
        elif port in email_ports:
            return 'email'
        elif port in remote_ports:
            return 'remote_access'
        else:
            return 'other'
    
    def generate_ipfix_header(self, length, is_data_record=False):
        version = 10
        export_time = int(time.time())
        sequence_number = self.sequence_number if is_data_record else 0
        
        header = struct.pack('!HHLLL',
            version,
            length,
            export_time,
            sequence_number,
            self.observation_domain_id
        )
        
        if is_data_record:
            self.sequence_number += 1
        
        return header

    def generate_ipfix_template(self):
        template_id = 257
        field_count = 16
        
        # Pack the field specifiers first to get the correct length
        field_specifiers = struct.pack('!HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH',
            8, 4,   # sourceIPv4Address
            12, 4,  # destinationIPv4Address
            7, 2,   # sourceTransportPort
            11, 2,  # destinationTransportPort
            4, 1,   # protocolIdentifier
            5, 1,   # ipClassOfService (optional)
            2, 4,   # inPacketDeltaCount
            24, 4,  # outPacketDeltaCount
            1, 4,   # inOctetDeltaCount
            23, 4,  # outOctetDeltaCount
            152, 8, # flowStartMilliseconds (optional ASA only)
            153, 8, # flowEndMilliseconds
            10, 4,  # ingressInterface
            14, 4,  # egressInterface
            95, 4,  # applicationId
            94, 2  # applicationName
        )
        
        # Calculate the total length of the template set
        total_length = 4 + 4 + len(field_specifiers)  # 4 bytes for set header, 4 bytes for template header
        
        template_set_header = struct.pack('!HH', 2, total_length)  # FlowSet ID = 2 for templates
        template_header = struct.pack('!HH', template_id, field_count)
        
        return template_set_header + template_header + field_specifiers

    def generate_ipfix_data_record(self):

        # NBAR application data 
        # Source: https://www.cisco.com/c/en/us/td/docs/ios-xml/ios/qos_nbar/prot_lib/config_library/pp6000/nbar-prot-pack6000/nbar-prot-pack6000_CLT_chapter.html
        nbar_applications = [
            (1454, "facebook", "Social Networking"),
            (82, "youtube", "Streaming Media"),
            (3, "http", "Web"),
            (1312, "ssl", "Network Protocol"),
            (1453, "twitter", "Social Networking"),
            (1456, "google-services", "Cloud and Data Center"),
            (72, "dns", "Network Protocol"),
            (1217, "slack", "Collaboration"),
            (31, "ntp", "Network Protocol"),
            (40, "ssh", "Network Protocol"),
            (1431, "ms-office-365", "Cloud and Data Center"),
            (1306, "webex-meeting", "Collaboration"),
            (1316, "netflix", "Streaming Media"),
            (1488, "whatsapp", "Social Networking"),
            (69, "bittorrent", "File Sharing")
        ]

        if self.current_flow_index >= len(self.flows):
            self.current_flow_index = 0
            self.update_signal.emit("End of flow array reached, replaying generated flow series.")

        flow = self.flows[self.current_flow_index]
        self.current_flow_index += 1

        src_ip = socket.inet_aton(flow['src_ip'])
        dst_ip = socket.inet_aton(flow['dst_ip'])
        src_port = random.randint(1024, 65535)
        dst_port = flow['dst_port']
        protocol = flow['protocol']
        ingress_interface = flow['ingress_interface']
        egress_interface = flow['egress_interface']
        
        # Track profile for this flow
        profile = self.categorize_flow_by_port(dst_port)
        self.profile_counts[profile] += 1

        # Use realistic traffic volumes based on service type
        inpackets, inoctets = get_realistic_traffic_volume(dst_port, protocol)
        outpackets, outoctets = get_realistic_traffic_volume(dst_port, protocol)
        
        # For some protocols, response is smaller (more realistic)
        if dst_port in [80, 443, 8080, 8443]:  # Web traffic
            # Request is smaller, response is larger
            outpackets = int(outpackets * 0.3)
            outoctets = int(outoctets * 0.3)
        elif dst_port in [53]:  # DNS
            # Query is tiny, response is small
            outpackets = 1
            outoctets = random.randint(50, 200)
        elif dst_port in [3306, 5432, 1433, 1521]:  # Databases
            # Query is smaller, result set can be large
            outpackets = int(outpackets * 0.4)
            outoctets = int(outoctets * 0.4)
        
        tos = random.choice([0, 0, 0, 16, 32, 40])  # Common ToS values (mostly 0)
        
        current_time_ms = int(time.time() * 1000)
        # Duration varies by service
        if dst_port in [53, 123]:  # DNS, NTP - very quick
            duration = random.randint(10, 500)
        elif dst_port in [80, 443, 8080, 8443]:  # Web - varies
            duration = random.randint(100, 30000)
        elif dst_port in [22, 3389]:  # Remote access - longer
            duration = random.randint(1000, 300000)
        else:  # Default
            duration = random.randint(1000, 60000)
        
        start_time = current_time_ms - duration
        end_time = current_time_ms

        application_id, application_name, application_category = random.choice(nbar_applications)
        
        record = struct.pack('!4s4sHHBBIIIIQQIIIH',
            src_ip,
            dst_ip,
            src_port,
            dst_port,
            protocol,
            tos,
            inpackets,
            outpackets,
            inoctets,
            outoctets,
            start_time,
            end_time,
            ingress_interface,
            egress_interface,
            application_id,
            len(application_name)
        )

        decoded_flow = {
            'src_ip': socket.inet_ntoa(src_ip),
            'dst_ip': socket.inet_ntoa(dst_ip),
            'src_port': src_port,
            'dst_port': dst_port,
            'protocol': str(protocol),
            'application': application_name,
            'application_category': application_category,
            'tos': tos,
            'in_packets': inpackets,
            'out_packets': inpackets,
            'in_octets': inoctets,
            'out_octets': inoctets,
            'start_time': time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time / 1000)),
            'end_time': time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end_time / 1000))
        }
        self.flow_signal.emit(decoded_flow)

        return (record) 

    def send_ipfix(self, source_ips, destination_ip, destination_port, num_flows, interval, packets_per_interval, flows_per_packet):
        flowset_id = 257
        while self.is_running and (num_flows == 0 or num_flows > 0):
            current_time = time.time()
            
            # Send template if it's time
            if current_time - self.last_template_time >= self.template_refresh_interval:
                for source_ip in source_ips:
                    template = self.generate_ipfix_template()
                    template_length = len(template) + 16  # 16 is the size of the IPFIX header
                    template_header = self.generate_ipfix_header(template_length, is_data_record=False)
                    template_packet = IP(src=source_ip, dst=destination_ip) / UDP(sport=random.randint(1024, 65535), dport=destination_port) / Raw(load=template_header + template)
                    send(template_packet, verbose=False)
                    self.update_signal.emit(f"Sent IPFIX template from {source_ip} to {destination_ip}:{destination_port}")
                self.last_template_time = current_time

            # Send data flows
            flows_sent = 0
            while flows_sent < packets_per_interval:
                if not self.is_running or (num_flows > 0 and num_flows <= 0):
                    break
                
                # Determine number of flows for this packet
                if flows_per_packet == 0:
                    packet_flows = random.randint(1, 10)
                else:
                    packet_flows = min(flows_per_packet, packets_per_interval - flows_sent)
                
                data_records = b''
                for _ in range(packet_flows):
                    data_records += self.generate_ipfix_data_record()
                
                data_set_header = struct.pack('!HH', flowset_id, len(data_records) + 4)  # +4 for the set header itself
                total_length = 16 + len(data_set_header) + len(data_records)  # 16 is the size of the IPFIX header
                header = self.generate_ipfix_header(total_length, is_data_record=True)
                payload = header + data_set_header + data_records

                for source_ip in source_ips:
                    pkt = IP(src=source_ip, dst=destination_ip) / UDP(sport=random.randint(1024, 65535), dport=destination_port) / Raw(load=payload)
                    send(pkt, verbose=False)
                    self.update_signal.emit(f"Sent IPFIX packet with {packet_flows} flows from {source_ip} to {destination_ip}:{destination_port}")

                flows_sent += packet_flows
                self.total_flows_sent += packet_flows * len(source_ips)
                self.flows_since_last_rate += packet_flows * len(source_ips)
                self.packets_sent += len(source_ips)
                self.stats_signal.emit(self.total_flows_sent)

                if num_flows > 0:
                    num_flows -= packet_flows
            
            # Calculate and emit rates every interval
            current_time = time.time()
            time_elapsed = current_time - self.last_rate_time
            if time_elapsed >= 1.0:  # Update rates every second
                flows_per_sec = self.flows_since_last_rate / time_elapsed
                packets_per_sec = self.packets_sent / time_elapsed
                self.rate_signal.emit(flows_per_sec, packets_per_sec)
                self.flows_since_last_rate = 0
                self.packets_sent = 0
                self.last_rate_time = current_time
                
                # Emit profile statistics
                self.profile_signal.emit(dict(self.profile_counts))

            time.sleep(interval)

        self.update_signal.emit("Finished sending IPFIX traffic")

    def run(self):
        self.send_ipfix(self.source_ips, self.destination_ip, self.destination_port, self.num_flows, self.interval, self.packets_per_interval, self.flows_per_packet)

    def stop(self):
        self.is_running = False

class IPFIXGeneratorGUI(QWidget):
    def __init__(self, cli_args=None):
        super().__init__()
        
        # Set window attributes for macOS compatibility
        self.setAttribute(Qt.WA_DeleteOnClose)
        
        self.cli_args = cli_args
        self.flows = []
        self.total_flows_sent = 0
        self.start_time = None
        self.profile_stats = {
            'web': 0, 'infrastructure': 0, 'database': 0,
            'email': 0, 'remote_access': 0, 'other': 0
        }
        self.initUI()

    def initUI(self):
        main_layout = QVBoxLayout(self)

        # Create a vertical splitter for the entire layout
        main_splitter = QSplitter(Qt.Vertical)
        
        # Add status bar
        self.status_bar = QStatusBar()
        self.status_bar.setFixedHeight(40)  # Increase status bar height
        
        # Add logo to status bar (left-aligned and larger)
        logo_label = QLabel()
        logo_pixmap = self.get_logo()
        if logo_pixmap:
            logo_label.setPixmap(logo_pixmap.scaledToHeight(30, Qt.SmoothTransformation))
            logo_label.setContentsMargins(5, 0, 5, 0)  # Add some horizontal padding
        self.status_bar.addWidget(logo_label)
        
        # Add a stretch to push other items to the right
        self.status_bar.addPermanentWidget(QWidget(), 1)

        self.source_hosts_label = QLabel("Source Hosts: 0")
        self.total_flows_label = QLabel("Total Flows Sent: 0")
        self.status_bar.addPermanentWidget(self.source_hosts_label)
        self.status_bar.addPermanentWidget(self.total_flows_label)
        main_layout.addWidget(self.status_bar)

        # Top section
        top_widget = QWidget()
        top_layout = QHBoxLayout(top_widget)

        # Left side: Input fields
        left_widget = QWidget()
        left_layout = QVBoxLayout(left_widget)
        self.add_input_fields(left_layout)

        # Button layout
        button_layout = QHBoxLayout()
        self.start_button = QPushButton("Start")
        self.start_button.clicked.connect(self.start_generation)
        button_layout.addWidget(self.start_button)

        self.stop_button = QPushButton("Stop")
        self.stop_button.clicked.connect(self.stop_generation)
        self.stop_button.setEnabled(False)
        button_layout.addWidget(self.stop_button)

        self.clear_button = QPushButton("Clear")
        self.clear_button.clicked.connect(self.clear_screen)
        button_layout.addWidget(self.clear_button)

        self.export_button = QPushButton("Export Flows")
        self.export_button.clicked.connect(self.export_flows)
        self.export_button.setEnabled(False)
        button_layout.addWidget(self.export_button)

        left_layout.addLayout(button_layout)

        # Right side: Log output
        self.log_output = QTextEdit()
        self.log_output.setReadOnly(True)

        # Add widgets to the top layout
        top_layout.addWidget(left_widget)
        top_layout.addWidget(self.log_output)

        # Add top widget to the main splitter
        main_splitter.addWidget(top_widget)

        # Flow table
        self.flow_table = QTableWidget()
        self.flow_table.setColumnCount(11)
        self.flow_table.setHorizontalHeaderLabels(['Src IP', 'Dst IP', 'Src Port', 'Dst Port', 'Protocol', 
                                                   'Application', 'Category' ,'ToS', 'Packets', 'Octets', 'Start Time', 'End Time'])
        self.flow_table.setSortingEnabled(True)  # Enable sorting
        self.flow_table.horizontalHeader().setStretchLastSection(True)
        self.flow_table.setAlternatingRowColors(True)
        main_splitter.addWidget(self.flow_table)

        # Set the main layout
        main_layout.addWidget(main_splitter)
        
        # Add statistics panel at the bottom
        self.create_statistics_panel(main_layout)

        # Set window properties
        self.setWindowTitle('LogicMonitor Demo Engineering: IPFIX Generator')
        self.resize(1200, 900)  # Increased size for statistics panel

    def get_logo(self):
        try:
            # Decode the base64 string to image data
            image_data = base64.b64decode(LOGO_BASE64)
            
            # Create a QImage from the image data
            image = QImage()
            image.loadFromData(image_data)
            
            # Convert QImage to QPixmap
            pixmap = QPixmap.fromImage(image)
            
            if pixmap.isNull():
                print("Failed to load logo")
                return None
            
            return pixmap
        except Exception as e:
            print(f"Error loading logo: {e}")
            return None

    def add_input_fields(self, layout):
        # Source IPs
        source_layout = QHBoxLayout()
        source_layout.addWidget(QLabel("Source IPs (comma-separated):"))
        self.source_ips = QLineEdit(getattr(self.cli_args, 'source', None) or "")
        source_layout.addWidget(self.source_ips)
        layout.addLayout(source_layout)

        # Destination IP
        dest_layout = QHBoxLayout()
        dest_layout.addWidget(QLabel("Destination IP:"))
        self.dest_ip = QLineEdit(getattr(self.cli_args, 'destination', None) or "")
        dest_layout.addWidget(self.dest_ip)
        layout.addLayout(dest_layout)

        # Destination Port
        dest_port_layout = QHBoxLayout()
        dest_port_layout.addWidget(QLabel("Destination Port:"))
        self.dest_port = QSpinBox()
        self.dest_port.setRange(1, 65535)
        self.dest_port.setValue(getattr(self.cli_args, 'port', None) or 2055)
        dest_port_layout.addWidget(self.dest_port)
        layout.addLayout(dest_port_layout)

        # Number of Flows
        flows_layout = QHBoxLayout()
        flows_layout.addWidget(QLabel("Number of Flows (0 for infinite):"))
        self.num_flows = QSpinBox()
        self.num_flows.setRange(0, 1000000)
        self.num_flows.setValue(getattr(self.cli_args, 'flows', None) or 0)
        flows_layout.addWidget(self.num_flows)
        layout.addLayout(flows_layout)

        # Interval
        interval_layout = QHBoxLayout()
        interval_layout.addWidget(QLabel("Interval (seconds):"))
        self.interval = QDoubleSpinBox()
        self.interval.setRange(0.1, 60)
        self.interval.setSingleStep(0.1)
        self.interval.setValue(getattr(self.cli_args, 'interval', None) or 1.0)
        interval_layout.addWidget(self.interval)
        layout.addLayout(interval_layout)

        # Flows per Interval
        packets_per_interval_layout = QHBoxLayout()
        packets_per_interval_layout.addWidget(QLabel("Packets per Interval:"))
        self.packets_per_interval = QSpinBox()
        self.packets_per_interval.setRange(1, 1000)
        self.packets_per_interval.setValue(getattr(self.cli_args, 'packets_per_interval', None) or 5)
        packets_per_interval_layout.addWidget(self.packets_per_interval)
        layout.addLayout(packets_per_interval_layout)

        # Flows per Packet
        flows_per_packet_layout = QHBoxLayout()
        flows_per_packet_layout.addWidget(QLabel("Flows per Packet (0 for random 1-10):"))
        self.flows_per_packet = QSpinBox()
        self.flows_per_packet.setRange(0, 1000)
        self.flows_per_packet.setValue(getattr(self.cli_args, 'flows_per_packet', None) or 0)
        flows_per_packet_layout.addWidget(self.flows_per_packet)
        layout.addLayout(flows_per_packet_layout)

        # Flow Source IPs
        flow_src_ips_layout = QVBoxLayout()
        flow_src_ips_layout.addWidget(QLabel("Flow Source IPs (comma-separated, CIDR allowed):"))
        self.flow_src_ips = QTextEdit()
        self.flow_src_ips.setPlaceholderText("Enter IP addresses or CIDR ranges, one per line or comma-separated")
        self.flow_src_ips.setFixedHeight(80)  # Adjust height as needed
        flow_src_default = getattr(self.cli_args, 'flow_src_ips', None)
        self.flow_src_ips.setPlainText(flow_src_default if flow_src_default else "192.168.1.0/24")
        flow_src_ips_layout.addWidget(self.flow_src_ips)
        layout.addLayout(flow_src_ips_layout)

        # Flow Destination IPs
        flow_dst_ips_layout = QVBoxLayout()
        flow_dst_ips_layout.addWidget(QLabel("Flow Destination IPs (comma-separated, CIDR allowed):"))
        self.flow_dst_ips = QTextEdit()
        self.flow_dst_ips.setPlaceholderText("Enter IP addresses or CIDR ranges, one per line or comma-separated")
        self.flow_dst_ips.setFixedHeight(80)  # Adjust height as needed
        flow_dst_default = getattr(self.cli_args, 'flow_dst_ips', None)
        self.flow_dst_ips.setPlainText(flow_dst_default if flow_dst_default else "54.193.15.255/32,54.194.232.54/32,54.209.7.170/32,54.254.224.41/32,69.25.43.0/24")
        flow_dst_ips_layout.addWidget(self.flow_dst_ips)
        layout.addLayout(flow_dst_ips_layout)

    def create_statistics_panel(self, layout):
        """Create comprehensive statistics panel"""
        stats_group = QGroupBox("Real-time Statistics & Traffic Profiles")
        stats_layout = QHBoxLayout()
        
        # Left side: Real-time metrics
        metrics_layout = QGridLayout()
        
        # Labels for metrics
        metrics_layout.addWidget(QLabel("<b>Real-time Metrics</b>"), 0, 0, 1, 2)
        
        metrics_layout.addWidget(QLabel("Flows/sec:"), 1, 0)
        self.flows_per_sec_label = QLabel("0.0")
        metrics_layout.addWidget(self.flows_per_sec_label, 1, 1)
        
        metrics_layout.addWidget(QLabel("Packets/sec:"), 2, 0)
        self.packets_per_sec_label = QLabel("0")
        metrics_layout.addWidget(self.packets_per_sec_label, 2, 1)
        
        metrics_layout.addWidget(QLabel("Uptime:"), 3, 0)
        self.uptime_label = QLabel("00:00:00")
        metrics_layout.addWidget(self.uptime_label, 3, 1)
        
        metrics_layout.addWidget(QLabel("Total Flows:"), 4, 0)
        self.stats_total_flows_label = QLabel("0")
        metrics_layout.addWidget(self.stats_total_flows_label, 4, 1)
        
        stats_layout.addLayout(metrics_layout)
        
        # Right side: Traffic profiles
        profiles_layout = QVBoxLayout()
        profiles_layout.addWidget(QLabel("<b>Traffic Distribution (Realistic Profiles)</b>"))
        
        # Create progress bars for each profile
        self.profile_bars = {}
        profiles = [
            ('web', 'Web (HTTP/HTTPS)', 50, '#4CAF50'),
            ('infrastructure', 'Infrastructure (DNS/NTP)', 20, '#2196F3'),
            ('database', 'Database (SQL)', 10, '#FF9800'),
            ('email', 'Email (SMTP/IMAP)', 8, '#9C27B0'),
            ('remote_access', 'Remote (SSH/RDP)', 7, '#F44336'),
            ('other', 'Other Services', 5, '#607D8B')
        ]
        
        for profile_key, profile_name, expected_pct, color in profiles:
            profile_layout = QHBoxLayout()
            label = QLabel(f"{profile_name}:")
            label.setFixedWidth(180)
            profile_layout.addWidget(label)
            
            bar = QProgressBar()
            bar.setMaximum(100)
            bar.setValue(0)
            bar.setFormat("%p% (0)")
            bar.setStyleSheet(f"""
                QProgressBar {{
                    border: 1px solid grey;
                    border-radius: 3px;
                    text-align: center;
                }}
                QProgressBar::chunk {{
                    background-color: {color};
                }}
            """)
            profile_layout.addWidget(bar)
            
            self.profile_bars[profile_key] = bar
            profiles_layout.addLayout(profile_layout)
        
        stats_layout.addLayout(profiles_layout)
        
        stats_group.setLayout(stats_layout)
        layout.addWidget(stats_group)
        
        # Timer for uptime updates (will start when generation begins)
        self.uptime_timer = QTimer()
        self.uptime_timer.timeout.connect(self.update_uptime)

    def update_uptime(self):
        """Update uptime display"""
        if self.start_time:
            elapsed = int(time.time() - self.start_time)
            hours = elapsed // 3600
            minutes = (elapsed % 3600) // 60
            seconds = elapsed % 60
            self.uptime_label.setText(f"{hours:02d}:{minutes:02d}:{seconds:02d}")
    
    def update_rates(self, flows_per_sec, packets_per_sec):
        """Update flow and packet rate displays"""
        self.flows_per_sec_label.setText(f"{flows_per_sec:.1f}")
        self.packets_per_sec_label.setText(f"{int(packets_per_sec)}")
    
    def update_profile_stats(self, profile_counts):
        """Update traffic profile distribution"""
        total = sum(profile_counts.values())
        if total == 0:
            return
        
        for profile_key, bar in self.profile_bars.items():
            count = profile_counts.get(profile_key, 0)
            percentage = int((count / total) * 100)
            bar.setValue(percentage)
            bar.setFormat(f"{percentage}% ({count})")
        
        self.profile_stats = profile_counts.copy()
        self.stats_total_flows_label.setText(str(total))

    def clear_screen(self):
        # Clear the flow table
        self.flow_table.setRowCount(0)
        self.flows = []

        # Clear the log output
        self.log_output.clear()

        #Clear status bar
        self.total_flows_label.setText("Total Flows Sent: 0")
        self.source_hosts_label.setText(f"Source Hosts: 0")
        
        # Clear statistics
        self.uptime_timer.stop()
        self.start_time = None
        self.uptime_label.setText("00:00:00")
        self.flows_per_sec_label.setText("0.0")
        self.packets_per_sec_label.setText("0")
        self.stats_total_flows_label.setText("0")
        for bar in self.profile_bars.values():
            bar.setValue(0)
            bar.setFormat("%p% (0)")

    def start_generation(self):
        # Clear the flow table
        self.flow_table.setRowCount(0)
        self.flows = []

        # Clear the log output
        self.log_output.clear()

        # Reset total flows sent
        self.total_flows_sent = 0
        self.total_flows_label.setText("Total Flows Sent: 0")

        source_ips = [ip.strip() for ip in self.source_ips.text().split(',')]
        self.source_hosts_label.setText(f"Source Hosts: {len(source_ips)}")

        dest_ip = self.dest_ip.text()
        dest_port = self.dest_port.value()
        num_flows = self.num_flows.value()
        interval = self.interval.value()
        packets_per_interval = self.packets_per_interval.value()
        flows_per_packet = self.flows_per_packet.value()

        flow_src_ips = parse_ip_ranges(self.flow_src_ips.toPlainText()) if self.flow_src_ips.toPlainText() else []
        flow_dst_ips = parse_ip_ranges(self.flow_dst_ips.toPlainText()) if self.flow_dst_ips.toPlainText() else []
        
        # Generate flows
        num_flows_to_generate = 1000  # Generate at least 10000 flows or the specified number
        flows = generate_flows(flow_src_ips, flow_dst_ips, num_flows_to_generate)  # Assuming TCP (protocol 6)
        self.update_log(f"Generated {num_flows_to_generate} sample flows for generation")

        self.generator_thread = IPFIXGeneratorThread(source_ips, dest_ip, dest_port, num_flows, interval, packets_per_interval, flows_per_packet, flow_src_ips, flow_dst_ips, flows)
        self.generator_thread.update_signal.connect(self.update_log)
        self.generator_thread.flow_signal.connect(self.add_flow_to_table)
        self.generator_thread.stats_signal.connect(self.update_total_flows)
        self.generator_thread.rate_signal.connect(self.update_rates)
        self.generator_thread.profile_signal.connect(self.update_profile_stats)
        self.generator_thread.start()
        
        # Start timing
        self.start_time = time.time()
        self.uptime_timer.start(1000)  # Update uptime every second

        self.start_button.setEnabled(False)
        self.stop_button.setEnabled(True)
        self.export_button.setEnabled(True)

        self.update_log("Started IPFIX traffic generation with realistic traffic profiles")

    def update_total_flows(self, total_flows):
        self.total_flows_sent = total_flows
        self.total_flows_label.setText(f"Total Flows Sent: {self.total_flows_sent}")

    def stop_generation(self):
        if hasattr(self, 'generator_thread'):
            self.generator_thread.stop()
            self.generator_thread.wait()
        
        # Stop uptime timer
        self.uptime_timer.stop()

        self.start_button.setEnabled(True)
        self.stop_button.setEnabled(False)

    def update_log(self, message):
        self.log_output.append(message)

    def add_flow_to_table(self, flow):
        row_position = self.flow_table.rowCount()
        self.flow_table.insertRow(row_position)
        
        # Determine color based on port (traffic profile)
        dst_port = flow.get('dst_port', 0)
        if dst_port in [80, 443, 8080, 8443]:  # Web
            color = QColor(76, 175, 80, 30)  # Light green
        elif dst_port in [53, 123, 514]:  # Infrastructure
            color = QColor(33, 150, 243, 30)  # Light blue
        elif dst_port in [3306, 5432, 1433, 1521]:  # Database
            color = QColor(255, 152, 0, 30)  # Light orange
        elif dst_port in [25, 587, 143, 993, 110, 995]:  # Email
            color = QColor(156, 39, 176, 30)  # Light purple
        elif dst_port in [22, 3389]:  # Remote access
            color = QColor(244, 67, 54, 30)  # Light red
        else:  # Other
            color = QColor(96, 125, 139, 30)  # Light blue-grey
        
        for col, (key, value) in enumerate(flow.items()):
            item = QTableWidgetItem(str(value))
            item.setBackground(QBrush(color))
            self.flow_table.setItem(row_position, col, item)
        
        self.flows.append(flow)
        
        # Auto-scroll to show latest flows
        self.flow_table.scrollToBottom()

    def export_flows(self):
        filename, _ = QFileDialog.getSaveFileName(self, "Export Flows", "", "CSV Files (*.csv)")
        if filename:
            with open(filename, 'w', newline='') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=self.flows[0].keys() if self.flows else [])
                writer.writeheader()
                for flow in self.flows:
                    writer.writerow(flow)
            self.update_log(f"Exported {len(self.flows)} flows to {filename}")

def parse_arguments():
    parser = argparse.ArgumentParser(description="Generate fake IPFIX traffic")
    parser.add_argument("--source", help="Comma-separated list of source IP addresses to spoof")
    parser.add_argument("--destination", help="Destination IP address")
    parser.add_argument("--port", type=int, default=2055, help="Destination port (default: 2055)")
    parser.add_argument("--flows", type=int, default=0, help="Number of flows to generate (0 for infinite)")
    parser.add_argument("--interval", type=float, default=0.5, help="Interval between flow generations in seconds")
    parser.add_argument("--packets-per-interval", type=int, default=5, help="Number of packets to send in each interval")
    parser.add_argument("--flows-per-packet", type=int, default=0, help="Number of flows per packet (0 for random 1-5)")
    parser.add_argument("--gui", action="store_true", help="Launch with GUI")
    parser.add_argument("--flow-src-ips", help="Comma-separated list of source IP addresses or CIDR ranges for flows")
    parser.add_argument("--flow-dst-ips", help="Comma-separated list of destination IP addresses or CIDR ranges for flows")
    return parser.parse_args()

if __name__ == '__main__':
    args = parse_arguments()
    
    if args.gui:
        app = QApplication(sys.argv)
        window = IPFIXGeneratorGUI(args)
        window.show()
        window.raise_()
        window.activateWindow()
        sys.exit(app.exec())
    else:
        # Validate required arguments
        if not args.source:
            print("Error: --source is required when running in CLI mode")
            sys.exit(1)
        if not args.destination:
            print("Error: --destination is required when running in CLI mode")
            sys.exit(1)
        
        # Parse source IPs
        source_ips = [ip.strip() for ip in args.source.split(',')]
        
        # Validate source IPs
        for ip in source_ips:
            try:
                ipaddress.IPv4Address(ip)
            except ValueError:
                print(f"Error: Invalid source IP address: {ip}")
                sys.exit(1)
        
        # Validate destination IP
        try:
            ipaddress.IPv4Address(args.destination)
        except ValueError:
            print(f"Error: Invalid destination IP address: {args.destination}")
            sys.exit(1)
        
        # Parse flow IP ranges with error handling
        try:
            flow_src_ips = parse_ip_ranges(args.flow_src_ips) if args.flow_src_ips else []
        except ValueError as e:
            print(f"Error parsing flow source IPs: {e}")
            sys.exit(1)
            
        try:
            flow_dst_ips = parse_ip_ranges(args.flow_dst_ips) if args.flow_dst_ips else []
        except ValueError as e:
            print(f"Error parsing flow destination IPs: {e}")
            sys.exit(1)
        
        # Generate flows before creating thread
        num_flows_to_generate = max(1000, args.flows) if args.flows > 0 else 1000
        print(f"Generating {num_flows_to_generate} sample flows...")
        flows = generate_flows(flow_src_ips, flow_dst_ips, num_flows_to_generate, show_progress=True)
        print(f"Generated {len(flows)} flows successfully")
        
        # Create generator thread with correct parameters
        generator = IPFIXGeneratorThread(
            source_ips,           # Fixed: was args.source (string), now source_ips (list)
            args.destination, 
            args.port, 
            args.flows, 
            args.interval, 
            args.packets_per_interval, 
            args.flows_per_packet, 
            flow_src_ips, 
            flow_dst_ips,
            flows                 # Fixed: Missing flows parameter
        )
        generator.update_signal.connect(print)  # Print updates to console
        
        # Set up signal handling for graceful shutdown
        shutdown_requested = [False]  # Use list to allow modification in nested function
        
        def signal_handler(sig, frame):
            print("\nShutdown requested... Please wait...")
            shutdown_requested[0] = True
            generator.stop()
        
        signal.signal(signal.SIGINT, signal_handler)
        signal.signal(signal.SIGTERM, signal_handler)
        
        print("Starting IPFIX generator... (Press Ctrl+C to stop)")
        generator.start()
        
        # Wait for thread with interruptible loop and status updates
        last_status_time = time.time()
        last_count = 0
        
        try:
            while generator.isRunning():
                if shutdown_requested[0]:
                    break
                
                # Show status every 5 seconds
                current_time = time.time()
                if current_time - last_status_time >= 5:
                    current_count = generator.total_flows_sent
                    flows_per_sec = (current_count - last_count) / 5 if last_count > 0 else 0
                    print(f"Status: {current_count} flows sent ({flows_per_sec:.1f} flows/sec)")
                    last_status_time = current_time
                    last_count = current_count
                
                time.sleep(0.1)  # Check every 100ms
            
            # Wait for thread to finish (with timeout)
            if generator.isRunning():
                generator.wait(5000)  # Wait up to 5 seconds
                
        except KeyboardInterrupt:
            print("\nForce stopping...")
            generator.stop()
            generator.wait(2000)  # Wait up to 2 seconds for cleanup
        
        # Show final statistics
        final_count = generator.total_flows_sent
        print(f"\nShutdown complete. Total flows sent: {final_count}")
        sys.exit(0)